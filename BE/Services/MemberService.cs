...
namespace BE.Services
{
    ...
    // - [MỚI - 19/07/2026] SẮP XẾP LẠI thứ tự các hàm trong file theo nhóm chức năng (helper riêng
    //   tư -> kiểm tra/truy vấn -> tạo hội viên -> cập nhật thông tin -> kích hoạt -> khóa/mở khóa
    //   -> lịch sử -> gia hạn -> hồ sơ tự xem) để dễ đọc/dễ maintain hơn. KHÔNG thay đổi logic của
    //   bất kỳ hàm nào khi di chuyển, chỉ đổi vị trí + thêm heading comment phân nhóm.
    // - [SỬA LỖI] CreateMemberAsync trước đây gọi thẳng BCrypt.Net.BCrypt.HashPassword(...) thay vì
    //   dùng chung PasswordHelper.HashPassword(...) như phần còn lại của hệ thống — không phải lỗi
    //   runtime nhưng gây KHÔNG ĐỒNG NHẤT cách hash mật khẩu giữa các luồng (nếu sau này đổi cost
    //   factor/thuật toán trong PasswordHelper thì chỗ này sẽ bị bỏ sót). Đã sửa lại dùng
    //   PasswordHelper.HashPassword để đồng bộ 100% với UpdateMyProfileAsync bên dưới.
    // - [MỚI] Thêm hàm UpdateMyProfileAsync — cho phép HỘI VIÊN tự cập nhật thông tin của chính
    //   mình (họ tên, số điện thoại, giới tính, mật khẩu). Đổi mật khẩu bắt buộc phải xác thực
    //   đúng CurrentPassword qua PasswordHelper.VerifyPassword trước khi cho đổi sang NewPassword
    //   (hash bằng PasswordHelper.HashPassword). Log đổi mật khẩu KHÔNG ghi giá trị thật (cũ/mới)
    //   vào MemberUpdateLog để tránh lộ thông tin nhạy cảm, chỉ ghi nhận "đã đổi mật khẩu".
    public class MemberService
    {
        ...
        // ===================== [PRIVATE] VALIDATE + LẤY PROMOTION (dùng chung cho luồng tạo/kích hoạt) =====================
        ...
        // ===================== [PRIVATE] SINH MẬT KHẨU NGẪU NHIÊN (dùng khi thu ngân tạo hội viên) =====================
        ...
        // ===================== [PRIVATE] SINH HÓA ĐƠN PDF NẾU GIAO DỊCH ĐÃ THANH TOÁN =====================
        ...
        // [SỬA LỖI] Đổi BCrypt.Net.BCrypt.HashPassword(...) trực tiếp -> PasswordHelper.HashPassword(...)
        // để đồng nhất cách hash mật khẩu với UpdateMyProfileAsync và toàn hệ thống.
            ...
            await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                {
                    var member = new Member
                    {
                        ...
                        Phone = request.Phone,
                        PasswordHash = PasswordHelper.HashPassword(generatedPassword), // [SỬA LỖI]
        ...
        // ===================== [NHÂN VIÊN] SỬA FACE ID / ẢNH ĐẠI DIỆN =====================
        ...
        // ===================== [MỚI] [HỘI VIÊN] TỰ CẬP NHẬT THÔNG TIN CỦA CHÍNH MÌNH =====================
        // Cho phép hội viên tự đổi: họ tên, số điện thoại, giới tính, mật khẩu.
        // - Các field FullName/Phone/Gender: optional, null/rỗng thì bỏ qua (giữ nguyên).
        // - Đổi mật khẩu: phải gửi CurrentPassword đúng với hash đang lưu (PasswordHelper.VerifyPassword)
        //   thì mới cho đổi sang NewPassword (được hash lại bằng PasswordHelper.HashPassword trước khi lưu DB —
        //   KHÔNG bao giờ lưu password dạng plain text).
        // - Không cho phép tự đổi Status/InternalNotes (những field đó chỉ nhân viên mới được sửa,
        //   dùng UpdateMemberInfoAsync).
        // - Log KHÔNG ghi giá trị thật của mật khẩu cũ/mới vào MemberUpdateLog, chỉ ghi nhận sự kiện
        //   "đã đổi mật khẩu" để tránh lộ thông tin nhạy cảm trong lịch sử cập nhật.
        public async Task<MemberResponse> UpdateMyProfileAsync(long memberId, UpdateMyProfileRequest request)
        {
            ...
            void TrackChange(string field, string? oldValue, string? newValue)
            {
                logs.Add(new MemberUpdateLog
                {
                    ...
                    OldValue = oldValue,
                    NewValue = newValue ?? string.Empty,
                    UpdatedByEmployeeId = null, // hội viên tự thao tác, không phải nhân viên
                    UpdatedAt = now
                });
            }
            ...
            if (!string.IsNullOrWhiteSpace(request.Phone) && request.Phone != member.Phone)
            {
                var phoneExisted = await _context.Members.AnyAsync(m => m.Phone == request.Phone && m.MemberId != memberId);
                if (phoneExisted)
                    throw new InvalidOperationException($"Số điện thoại '{request.Phone}' đã được sử dụng.");
            ...
            if (!string.IsNullOrWhiteSpace(request.NewPassword))
            {
                ...
                if (!PasswordHelper.VerifyPassword(request.CurrentPassword, member.PasswordHash))
                ...
                // Không ghi giá trị mật khẩu thật vào log — chỉ ghi nhận sự kiện đã đổi mật khẩu.
                logs.Add(new MemberUpdateLog
                {
                    UpdateSessionId = sessionId,
                    MemberId = memberId,
