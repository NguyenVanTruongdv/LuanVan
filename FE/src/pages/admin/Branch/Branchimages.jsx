...
export default function BranchImages() {
    ...
    return (
        ...
        .bi-breadcrumb span.current { color: var(--cyan-dark); font-weight: 700; }
        ...
        .bi-branch-picker label { display: block; font-size: 12.5px; font-weight: 700; color: var(--muted); margin-bottom: 6px; }
        .bi-select-wrap { position: relative; }
        .bi-select-wrap svg.chev { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); color: var(--cyan-dark); pointer-events: none; }
        .bi-select-wrap select { width: 100%; appearance: none; border: 1.5px solid var(--line); border-radius: 10px; padding: 10px 34px 10px 14px; font-size: 14px; font-weight: 600; color: var(--ink); background: var(--card-bg); cursor: pointer; outline: none; }
        .bi-select-wrap select:focus { border-color: var(--cyan); box-shadow: 0 0 0 3px var(--cyan-soft); }
        .bi-branch-card { background: var(--card-bg); border: 1.5px solid var(--line); border-left: 5px solid var(--cyan); border-radius: 14px; padding: 18px 22px; display: flex; align-items: center; gap: 20px; margin-bottom: 22px; box-shadow: 0 4px 14px rgba(15, 23, 42, 0.06); flex-wrap: wrap; }
        .bi-branch-thumb { width: 84px; height: 84px; border-radius: 10px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; color: #FFFFFF; }
        ...
        .bi-branch-meta svg { color: var(--cyan-dark); flex-shrink: 0; }
        .bi-status-row { display: flex; align-items: center; gap: 8px; margin-top: 8px; font-size: 13px; color: var(--muted); font-weight: 500; }
        .bi-pill { font-size: 12px; font-weight: 700; padding: 3px 10px; border-radius: 999px; border: 1px solid transparent; }
        .bi-pill.active { background: var(--green-soft); color: var(--green); border-color: rgba(22, 163, 74, 0.35); }
        .bi-pill.inactive { background: rgba(100, 116, 139, 0.1); color: var(--muted); border-color: rgba(100, 116, 139, 0.25); }
        .bi-detail-btn { display: flex; align-items: center; gap: 6px; border: 1.5px solid var(--line); background: var(--input-bg); color: var(--ink); border-radius: 10px; padding: 9px 16px; font-size: 13px; font-weight: 600; cursor: pointer; white-space: nowrap; }
        .bi-detail-btn:hover { border-color: var(--cyan); color: var(--cyan-dark); }
        .bi-layout { display: grid; grid-template-columns: 0.85fr 1.3fr; gap: 20px; align-items: start; }
        .bi-card { background: var(--card-bg); border: 1.5px solid var(--line); border-top: 3px solid var(--cyan); border-radius: 14px; padding: 24px; box-shadow: 0 4px 14px rgba(15, 23, 42, 0.06); }
        ...
        .bi-dropzone svg { color: var(--cyan-dark); }
        .bi-dropzone-title { font-size: 14px; font-weight: 600; color: var(--ink); }
        .bi-dropzone-btn { margin-top: 4px; border: 1.5px solid var(--cyan); color: var(--cyan-dark); background: transparent; border-radius: 8px; padding: 8px 18px; font-size: 13px; font-weight: 700; cursor: pointer; }
        .bi-dropzone-btn:hover { background: var(--cyan-soft); }
        .bi-dropzone-meta { font-size: 12px; color: var(--muted); font-weight: 500; }
        .bi-legend { margin-top: 18px; background: var(--input-bg); border: 1.5px solid var(--line); border-radius: 12px; padding: 14px 16px; }
        .bi-legend-title { display: flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 700; color: var(--ink); margin-bottom: 8px; }
        .bi-legend-title svg { color: var(--cyan-dark); }
        ...
        .bi-add-btn { display: flex; align-items: center; gap: 6px; background: var(--cyan); color: #FFFFFF; border: 1.5px solid var(--cyan); border-radius: 10px; padding: 9px 16px; font-size: 13px; font-weight: 700; cursor: pointer; }
        .bi-add-btn:hover { background: var(--cyan-dark); border-color: var(--cyan-dark); }
        .bi-empty { text-align: center; padding: 40px 20px; color: var(--muted); font-size: 13.5px; font-weight: 500; border: 1.5px dashed var(--line); border-radius: 12px; }
        .bi-table { width: 100%; border-collapse: collapse; border: 1.5px solid var(--line); border-radius: 10px; overflow: hidden; }
        .bi-table th { text-align: left; font-size: 12px; text-transform: uppercase; letter-spacing: 0.04em; color: var(--cyan-dark); font-weight: 700; padding: 10px; background: var(--cyan-soft); border-bottom: 1.5px solid var(--line); }
        ...
        .bi-table tr:hover td { background: var(--input-bg); }
        ...
        .bi-thumb { width: 56px; height: 44px; border-radius: 8px; overflow: hidden; flex-shrink: 0; display: flex; align-items: center; justify-content: center; color: var(--muted); background: var(--input-bg); border: 1px solid var(--line); }
        .bi-thumb img { width: 100%; height: 100%; object-fit: cover; display: block; }
        .bi-type-label { font-weight: 600; color: var(--ink); }
        .bi-type-label.cover { color: var(--cyan-dark); }
        .bi-order-stepper { display: flex; align-items: center; gap: 8px; }
        .bi-step-btn { width: 26px; height: 26px; border-radius: 7px; border: 1.5px solid var(--line); background: var(--input-bg); color: var(--ink); display: flex; align-items: center; justify-content: center; cursor: pointer; }
        .bi-step-btn:hover:not(:disabled) { border-color: var(--cyan); color: var(--cyan-dark); }
        ...
        .bi-cover-badge { font-size: 11.5px; font-weight: 700; color: var(--cyan-dark); background: var(--cyan-soft); padding: 3px 9px; border-radius: 999px; border: 1px solid rgba(22, 163, 74, 0.35); }
        .bi-row-actions { display: flex; gap: 6px; }
        .bi-icon-btn { width: 30px; height: 30px; border-radius: 8px; border: 1.5px solid var(--line); background: var(--input-bg); color: var(--ink); display: flex; align-items: center; justify-content: center; cursor: pointer; }
        .bi-icon-btn:hover { border-color: var(--cyan); color: var(--cyan-dark); }
        .bi-icon-btn.danger:hover { border-color: var(--red); color: var(--red); background: rgba(220, 38, 38, 0.06); }
        .bi-actions-bar { display: flex; justify-content: center; gap: 10px; margin-top: 22px; }
        .bi-btn { border-radius: 10px; padding: 11px 22px; font-size: 14px; font-weight: 700; cursor: pointer; border: 1.5px solid var(--line); background: var(--card-bg); color: var(--ink); }
        .bi-btn:hover { background: var(--input-bg); border-color: var(--muted); }
        .bi-btn:disabled { opacity: 0.6; cursor: not-allowed; }
        .bi-btn-primary { background: var(--cyan); border-color: var(--cyan); color: #FFFFFF; }
        .bi-btn-primary:hover { background: var(--cyan-dark); border-color: var(--cyan-dark); color: #FFFFFF; }
        .bi-toast { max-width: 1400px; margin-top: 16px; background: var(--green-soft); border: 1.5px solid rgba(22, 163, 74, 0.4); color: var(--green); padding: 12px 16px; border-radius: 10px; font-size: 14px; font-weight: 600; text-align: center; }
        .bi-error { max-width: 1400px; margin-top: 16px; background: rgba(220,38,38,0.08); border: 1.5px solid rgba(220,38,38,0.35); color: var(--red); padding: 12px 16px; border-radius: 10px; font-size: 14px; font-weight: 600; text-align: center; }
        .bi-modal-overlay { position: fixed; inset: 0; background: rgba(15, 23, 42, 0.55); display: flex; align-items: center; justify-content: center; z-index: 50; padding: 20px; }
        .bi-modal { background: var(--card-bg); border: 1.5px solid var(--line); border-top: 4px solid var(--cyan); border-radius: 14px; padding: 24px; width: 100%; max-width: 380px; box-shadow: 0 20px 50px rgba(15, 23, 42, 0.25); }
        ...
        .bi-modal select, .bi-modal .bi-text-input { width: 100%; border: 1.5px solid var(--line); border-radius: 10px; padding: 10px 14px; font-size: 14px; font-weight: 500; color: var(--ink); background: var(--input-bg); margin-bottom: 16px; appearance: none; }
      ...
      `}</style>
            ...
            <div className="bi-branch-card">
                <div className="bi-branch-thumb" style={{ background: "linear-gradient(135deg, #16A34A, #15803D)" }}>
