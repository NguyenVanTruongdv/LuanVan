...
// Field thuộc nhóm "FaceID" — dùng để gắn tag màu xanh dương cho mỗi phiên
// cập nhật trong lịch sử. Field còn lại mặc định thuộc nhóm "thông tin cá
// nhân" (tag màu xanh lá).
const FACE_FIELDS = new Set(["FaceID"]);

function sessionTag(changes) {
    const isFaceChange = (changes || []).some((c) => FACE_FIELDS.has(c.fieldName));
    return isFaceChange
        ? { label: "Cập nhật FaceID", icon: ScanFace, variant: "face" }
...
const styles = `
  * { box-sizing: border-box; }
  ...
  /* Modal Lịch sử cập nhật — overlay tối, card trắng bo góc lớn, danh sách
     các phiên cập nhật là card nền màu xếp dọc. */
  .elp-history-overlay {
    position: fixed;
    inset: 0;
    z-index: 60;
  ...
  .elp-history-card--face { background: var(--elp-face-bg); border: 1px solid var(--elp-face-border); }
  ...
  .elp-history-tag--face { background: #FFFFFF; color: var(--elp-face-dark); }
...
/* ============================================================================
   ...
   tin cá nhân, xanh dương = FaceID), thời gian, người thực hiện, và danh
   sách field đã đổi (giá trị cũ gạch ngang -> giá trị mới).
   ============================================================================ */
...
/* ============================================================================
   ...
   không đụng tài khoản đăng nhập. Profile kỳ vọng có thêm 3 field:
   createdAt, createdBy, updatedAt (để hiển thị dòng meta phía trên 2 card).
   ============================================================================ */
...
/* ============================================================================
   ...
   Lịch sử cập nhật hiển thị dưới dạng modal (giống trang mẫu) thay vì panel
   trải phẳng trong trang.
   ============================================================================ */
        ...
        <div className="elp-page">
            ...
            <div className={`elp-wrap-narrow ${!isCreateMode ? "elp-wrap-wide" : ""}`}>
                <div className="elp-top-bar">
                    <button onClick={() => navigate("/admin/employees")} className="elp-back-btn">
                        <ArrowLeft size={16} />
                        Quay lại danh sách
                    </button>
