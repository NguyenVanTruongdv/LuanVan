                        ...
                        return (
                            <div key={s.sessionId} style={{ border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 14px", background: C.cardAlt }}>
                                ...
                                <div style={{ fontSize: 12, color: C.inkMuted, fontWeight: 600, marginBottom: 10 }}>
                                    {s.employeeName || "Nhân viên tự cập nhật"}
                                </div>
...
// ── Modal xác nhận ngưng gói nội bộ — gọi POST /api/members/{memberId}/packages/cancel-internal (không gửi body) ──
function CancelPackageModal({ memberName, packageName, onConfirm, onClose, submitting, error }) {
    return (
        <div className="gw-modal-backdrop" onClick={submitting ? undefined : onClose}>
            <div className="gw-modal" onClick={e => e.stopPropagation()}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                    <div style={{ width: 34, height: 34, borderRadius: 10, background: C.redBg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <AlertTriangle size={17} color={C.red} />
                    </div>
                    <span style={{ fontSize: 16.5, fontWeight: 800, color: C.ink }}>Ngưng gói nội bộ</span>
                </div>

                <div style={{ fontSize: 13.5, color: C.inkSoft, lineHeight: 1.6, marginBottom: 18 }}>
                    Xác nhận ngưng gói tập <b style={{ color: C.ink }}>{packageName}</b> của nhân viên{" "}
                    <b style={{ color: C.ink }}>{memberName}</b>? Hành động này sẽ dừng hiệu lực gói ngay lập tức.
                </div>

                {error && (
                    <div style={{ marginBottom: 14, padding: "10px 13px", borderRadius: 10, background: C.redBg, color: C.red, fontSize: 13, fontWeight: 700, border: `1px solid ${C.redBorder}` }}>
                        {error}
                    </div>
                )}
                ...
                <div style={{ display: "flex", gap: 10 }}>
                    <button
                        onClick={onConfirm}
                        disabled={submitting}
                        style={{ ...btnDanger, flex: 1, justifyContent: "center", opacity: submitting ? 0.6 : 1 }}
                    >
                        <X size={15} /> {submitting ? "Đang xử lý…" : "Xác nhận ngưng gói"}
                    </button>
                    <button onClick={onClose} disabled={submitting} style={{ ...btnCancelSoft, flex: "0 0 96px", justifyContent: "center" }}>
                        Hủy
                    </button>
                </div>
            </div>
    ...
    return (
        <div>
            <div style={{ marginBottom: 24 }}>
                <div style={{ fontSize: 26, fontWeight: 900, color: C.ink, letterSpacing: -0.5 }}>Danh sách Nhân viên</div>
                <div style={{ fontSize: 13.5, color: C.inkMuted, marginTop: 4, fontWeight: 600 }}>
                    {loading ? "Đang tải danh sách nhân viên…" : `${filtered.length} / ${members.length} nhân viên được hiển thị`}
                </div>
            </div>

            <div style={{ display: "flex", gap: 14, marginBottom: 22, flexWrap: "wrap" }}>
                <StatCard icon={Users} label="Tổng nhân viên" value={members.length} color={C.accentDark} bgColor={C.accentSoft} />
            ...
            <div style={{ border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden", background: C.card, boxShadow: C.shadow }}>
                <div className="gw-table-scroll">
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                        <thead>
                            <tr>
                                {["Nhân viên", "Số điện thoại", "Chi nhánh", "Gói tập", "Trạng thái", ""].map((h, i) => (
                                    <th key={i} style={{
...
function DetailPage({ memberId, listSnapshot, onBack, onSave, initialEditingInfo = false }) {
    ...
    // Ngưng gói nội bộ — khớp POST /api/members/{memberId}/packages/cancel-internal
    const [showCancelPackage, setShowCancelPackage] = useState(false);
    const [cancelingPackage, setCancelingPackage] = useState(false);
    const [cancelPackageError, setCancelPackageError] = useState("");
    ...
    const fetchDetail = useCallback(() => {
        ...
        return cashierApi.getMemberDetail(memberId)
            ...
            .catch(() => { setError("Không tải được thông tin chi tiết nhân viên."); })
    ...
    const handleSaveInfo = async () => {
        ...
        try {
            ...
            showFlash("Đã lưu thông tin nhân viên!");
        } catch (e) {
    ...
    // Gọi POST /api/members/{memberId}/packages/cancel-internal — không gửi body, chỉ có memberId
    const handleConfirmCancelPackage = async () => {
        setCancelPackageError("");
        setCancelingPackage(true);
        try {
            await managerApi.cancelInternalPackage(memberId);
            const updated = await fetchDetail();
            if (updated) onSave(updated);
            setShowCancelPackage(false);
            showFlash("Đã ngưng gói nội bộ!");
        } catch (e) {
            // Log lỗi thật ra console để debug (status code, message từ BE, v.v.)
            console.error("cancelInternalPackage failed:", e?.response?.status, e?.response?.data || e);
            const beMessage = e?.response?.data?.message || e?.response?.data?.error || e?.response?.data?.title;
            setCancelPackageError(beMessage || `Ngưng gói nội bộ thất bại${e?.response?.status ? ` (mã lỗi ${e.response.status})` : ""}. Vui lòng thử lại.`);
        } finally {
        ...
        <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 22, flexWrap: "wrap" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div>
                        <div style={{ fontSize: 22, fontWeight: 800, color: C.ink, letterSpacing: -0.4 }}>Chi tiết nhân viên</div>
                    ...
                    <div className="gw-photo-col" style={{
                        ...
                        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 13 }}>
                            {editingPhoto ? (
                                ...
                                <>
                                    <Eyebrow icon={Camera}>Ảnh / FaceID nhân viên</Eyebrow>
                                    <div style={{
                    ...
                    <div style={{ flex: "1 1 440px", minWidth: 0, padding: "28px 28px 24px", display: "flex", flexDirection: "column" }}>
                        <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                            <div style={{
                                ...
                                {!editingInfo && (
                                    <button
                                        onClick={() => { setCancelPackageError(""); setShowCancelPackage(true); }}
                                        style={{ ...btnDanger, padding: "7px 13px", fontSize: 12.5 }}
                                    >
                                        <X size={13} /> Ngưng gói nội bộ
                                    </button>
                                )}
                            </div>
            ...
            {showCancelPackage && (
                <CancelPackageModal
                    memberName={draft.hoTen}
                    packageName={draft.goiTap}
                    submitting={cancelingPackage}
                    error={cancelPackageError}
                    onConfirm={handleConfirmCancelPackage}
                    onClose={() => !cancelingPackage && setShowCancelPackage(false)}
                />
            )}
        </div>
    );
...
/* ── APP SHELL ── */
export default function ListEmployeeOfAdmin() {
