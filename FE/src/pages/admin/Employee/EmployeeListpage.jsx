...
/* ============================================================================
MODAL XÁC NHẬN CHO NGHỈ VIỆC
Không còn nhập lý do — chỉ hỏi xác nhận có/không.
============================================================================ */

function ConfirmHideModal({ fullName, onCancel, onConfirm }) {
        ...
        <div className="elp-overlay">
            <div className="elp-modal">
                ...
                <p className="elp-modal-desc">
                    Bạn có chắc chắn muốn cho <strong>{fullName}</strong> nghỉ việc?
                </p>
...
Route: /employees
- "Thêm nhân viên" -> điều hướng sang trang chi tiết ở chế độ tạo mới (/employees/new)
- "Xem" -> điều hướng sang trang chi tiết ở chế độ xem/sửa (/employees/:id)

Dùng adminApi cho luồng FACEID (không đụng Account):
- GET  api/employee?fullName=&status=&branchId=  -> adminApi.getEmployeeList
- GET  api/branches                              -> adminApi.getBranches
- PATCH api/employee/{id}/hide                   -> adminApi.hideEmployee (cho nghỉ việc, không còn cần reason)
- PATCH api/employee/{id}/activate               -> adminApi.activateEmployee (kích hoạt lại)
============================================================================ */
...
export default function EmployeeListPage() {
    ...
    const fetchList = useCallback(async (f) => {
        ...
        setListError("");
        try {
            // GET: api/employee?fullName=&phone=&branchId=&status=...
            const res = await adminApi.getEmployeeList({
                fullName: f.name || undefined,
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const rawBranches = res?.data ?? res;
    ...
    const toggleStatus = async (emp) => {
        ...
        try {
            ...
            setListError(err?.data?.message || err?.message || "Kích hoạt lại tài khoản thất bại.");
        } finally {
            setBusyId(null);
        }
    };
    ...
    const confirmHide = async () => {
        ...
        try {
            // PATCH: api/employee/{id}/hide — không còn truyền reason
            await adminApi.hideEmployee(confirmLock.employeeId);
            setConfirmLock(null);
            await fetchList(filters);
        } catch (err) {
            setListError(err?.data?.message || err?.message || "Cho nghỉ việc thất bại.");
            setConfirmLock(null);
        } finally {
            setBusyId(null);
        }
        ...
        <div className="elp-page">
            ...
            <div className="elp-wrap">
                <div className="elp-header">
                    ...
                    <button onClick={() => navigate("/admin/employees/create")} className="btn btn-primary">
                        <Plus size={16} />
                        Thêm nhân viên
                    </button>
                </div>
                ...
                <div className="elp-filters">
                    ...
                    <div className="elp-select-wrap">
                        ...
                        >
                            ...
                            <option value="Suspended">Nghỉ việc</option>
                        </select>
                            ...
                            <div key={emp.employeeId} className="elp-row-grid elp-trow">
                                ...
                                <div className="elp-actions">
                                    ...
                                    >
                                        ...
                                        {emp.status === "Active" ? "Nghỉ việc" : "Kích hoạt lại"}
                                    </button>
                                </div>
                            </div>
                        ))
