...
//   /branches có shape { items: [ { branchId, branchName, ... } ] }).
//   Code cũ gán thẳng response đó vào state rồi gọi .map() trên object ->
//   "TypeError: branches.map is not a function".
//   => Thêm hàm unwrapList() để tự nhận diện: nếu response đã là mảng thì
//      dùng luôn, nếu là object thì lấy field .items (fallback thêm .data /
//      .branches / .categories / .equipments / .results cho chắc), luôn trả
//      về mảng. Áp dụng cho cả categories, branches và danh sách thiết bị.
...
// `{ data: [...] }`,...). Luôn trả về mảng để .map()/.filter() không bao giờ
// crash, kể cả khi API lỗi/null/undefined.
// ---------------------------------------------------------------------------
...
function CustomSelect({ value, onChange, options, placeholder, disabled, getOptionValue, getOptionLabel, showAllOption = true }) {
    ...
    // options luôn phải là mảng — phòng trường hợp component cha lỡ truyền
    // nhầm object (chưa unwrap), tránh crash ngay tại đây.
...
export default function EquipmentListPageOfAdmin() {
    ...
    const loadContext = useCallback(async () => {
        ...
        // hoặc trả object phân trang { items: [...] } tuỳ endpoint -> luôn
        // unwrapList() trước khi setState để tránh crash ở .map().
        const catsRaw = await managerApi.getEquipmentCategories().catch(() => []);
        setCategories(unwrapList(catsRaw));

        let branchListRaw = [];
        if (currentRole === "Manager") {
            // profile.branches do BE trả kèm trong profile, không qua API
            // /branches riêng -> vẫn unwrap phòng trường hợp cũng bị bọc.
            branchListRaw = profile?.branches ?? [];
            if (unwrapList(branchListRaw).length === 0) {
                branchListRaw = await managerApi.getBranches().catch(() => []);
            }
        } else {
            branchListRaw = await managerApi.getBranches().catch(() => []);
        }
        const branchList = unwrapList(branchListRaw);
